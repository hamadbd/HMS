...
class MedicalProvider extends ChangeNotifier {
  ...
  getMedical() async {
    ...
    if (response.statusCode == 200) {
      ...
      if(listLab.isEmpty){
        ...
        isAny=true;
        getPatientById(listLab[0].patientId.toString());
      }
  ...
  updateMedicalResult(String patientId , String id,String result,BuildContext context) async {
    loading = true;
    notifyListeners();
    var url = "${URL.BASE_URL}/updateMedicalResult/${id}/${result}/${patientId}";
    print(url);
    final response = await http.get(Uri.parse(url));
    if (response.statusCode == 200) {
      ...
      Navigator.pushReplacement(context, PageTransition(type: PageTransitionType.bottomToTop, child: MedicalPage()));
    } else {
      loading = false;
      throw Exception("Failed to load data");
    }
